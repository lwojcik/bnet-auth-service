...
describe("AccessToken plugin", () => {
  ...
  it("exposes getFreshAccessToken() method", () => {
    expect(typeof fastifyServer.accessToken.getFreshAccessToken).toBe(
      "function"
    );
  });

  it("exposes getCachedAccessToken() method", () => {
    expect(typeof fastifyServer.accessToken.getCachedAccessToken).toBe(
      "function"
    );
  });

  it("exposes isAccessTokenCached() method", () => {
    expect(typeof fastifyServer.accessToken.isAccessTokenCached).toBe(
      "function"
    );
  });
